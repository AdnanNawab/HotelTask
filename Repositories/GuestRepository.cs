using Dapper;
using Hotel1.Models;
using Hotel1.Utilities;

namespace Hotel1.Repositories;
...
public class GuestRepository : BaseRepository, IGuestRepository
{
    ...
    public async  Task<Guest> GetById(int Id)
    {
        var query = $@"SELECT * FROM {TableNames.guest} WHERE id =@Id";
         ...
         using (var con = NewConnection)
         return await con.QueryFirstOrDefaultAsync<Guest>(query, new {Id});
    }
    ...
    public async Task<bool> Update(Guest Item)
    {
        ...
        SET name = @Name, mobile = @Mobile, email = @Email, date_of_birth =@DateOfBirth, address = @Address WHERE id = @Id";
